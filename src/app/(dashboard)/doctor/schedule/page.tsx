...
import { MASTER_SLOTS } from "@/utils/constants";
      ...
      <CardFooter className="p-8 pt-0 flex gap-3">
        {apt.status === "PENDING" ? (
          <>
            <Button
              variant="ghost"
              className="flex-1 h-12 rounded-xl font-black text-[10px] uppercase tracking-widest text-red-600 hover:bg-red-50"
              onClick={() => onStatusUpdate(apt.id, "REJECTED")}
            >
              <XCircle className="h-4 w-4 mr-2" /> Decline
            </Button>
            <Button
              className="flex-1 h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-100"
              onClick={() => onStatusUpdate(apt.id, "APPROVED")}
            >
          ...
          <Button
            variant="ghost"
            className="w-full h-12 rounded-xl bg-slate-50 text-slate-900 font-black text-[10px] uppercase tracking-widest group border border-slate-100 hover:bg-slate-100 transition-colors"
          >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <Card className="lg:col-span-5 border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white p-2">
        <CardHeader className="p-8">
          ...
          <CardDescription className="font-medium text-slate-400 text-xs">
            Manage your clinical consultation rates.
          </CardDescription>
        </CardHeader>
        <CardContent className="px-8 pb-4">
          <div className="space-y-4">
            <label className="block leading-none text-[10px] font-black uppercase tracking-widest text-slate-400">
              Standard Checkup Fee
            </label>
      ...
      <Card className="lg:col-span-7 border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white overflow-hidden">
        <CardHeader className="p-8">
          ...
          <CardDescription className="font-medium text-slate-400 text-xs">
            Define your daily window for digital and physical visits.
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {MASTER_SLOTS.map((time) => {
              ...
              return (
                <div
                  key={time}
                  className={`flex items-center justify-between p-5 rounded-2xl border-2 transition-all ${isActive ? "border-blue-100 bg-blue-50/20" : "border-slate-50 bg-slate-50/50 grayscale opacity-60"}`}
                >
                  <span
                    className={`font-black text-xs uppercase tracking-widest ${isActive ? "text-blue-900" : "text-slate-400"}`}
                  >
                    {time}
                  </span>
                  <Switch
...
export default function DoctorSchedulePage() {
  ...
  const handleStatusUpdate = useCallback(
    async (id: string, newStatus: "APPROVED" | "REJECTED" | "COMPLETED") => {
      if (!id) {
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in fade-in slide-in-from-top-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            ...
            {isLoading
              ? "Syncing data..."
              : `Managing ${appointments.length} active bookings.`}
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="appointments" className="w-full">
        <TabsList className="h-16 inline-flex bg-slate-100 rounded-[1.5rem] p-1.5 mb-8">
          <TabsTrigger
            value="appointments"
            className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm"
          >
            Active Bookings
          </TabsTrigger>
          <TabsTrigger
            value="availability"
            className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600"
          >
            Availability & Fees
          </TabsTrigger>
        </TabsList>
