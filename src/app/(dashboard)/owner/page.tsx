...
export default function OwnerDashboardPage() {
  ...
  const { stats, nextAppointment, recentOrders } = data || {
    stats: { totalPets: 0, upcomingCount: 0, activeOrders: 0 },
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <PawPrint className="h-4 w-4 text-orange-600" /> Welcome back,{" "}
          ...
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white">
            <CardHeader className="flex flex-row items-center justify-between p-8 bg-slate-50/50">
              <div>
                ...
                <CardDescription className="font-medium text-[10px] uppercase tracking-widest text-slate-400">
                  Status Overview
                </CardDescription>
              </div>
            </CardHeader>
                ...
                recentOrders.map((order: any) => (
                  <div key={order.id} className="group">
                    ...
                    <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${order.status === "SHIPPED" ? "bg-orange-500 w-2/3" : "bg-emerald-500 w-full"}`}
                      />
                    </div>
                  </div>
                ))
...
function MetricCard({
  ...
  return (
    <Card
      className={`border-none shadow-lg rounded-[2.5rem] ${highlight ? "ring-2 ring-orange-500/20" : ""}`}
    >
...
function ShortcutCard({ href, icon, title, sub, color }: any) {
  return (
    <Link href={href} className="block group">
      <Card
        className={`border-none shadow-none rounded-[2rem] transition-all bg-${color}-50/50 group-hover:bg-${color}-100/50`}
      >
