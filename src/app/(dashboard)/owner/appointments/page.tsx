...
import { specialties, timeSlots } from "@/utils/appoinments";

export default function VetAppointmentsPage() {
  const { user } = useAuthStore();
  const { doctorsQuery, petsQuery, createAppointment } = usePetOwnerBooking(
    user?.id,
  );
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctorsQuery.data.filter((doc) => {
      const matchesSearch = doc.full_name
        .toLowerCase()
  ...
  const handleConfirmBooking = async () => {
    ...
    createAppointment.mutate(
      ...
      },
      {
        ...
        onError: (err: any) => {
          toast.error(
            `Booking Failed: ${err.message || "Please check your network."}`,
          );
        },
      },
    );
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredDoctors.map((doc) => (
          <Card
            key={doc.id}
            className="border-none shadow-xl rounded-[2.5rem] bg-white transition-all hover:scale-[1.01]"
          >
            <CardHeader className="p-8 pb-4">
              <div className="flex gap-5 items-start">
                ...
                <div className="flex-1 space-y-1">
                  <CardTitle className="text-xl font-[900] uppercase tracking-tighter text-slate-900">
                    {doc.full_name}
                  </CardTitle>
            ...
            <CardContent className="p-8 pt-4">
              <div className="flex items-center text-[11px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 p-3 rounded-2xl">
                ...
                {doc.location || "Kolkata, WB"}
              </div>
            </CardContent>
            <CardFooter className="p-8 pt-0 flex items-center justify-between">
              <div>
                <span className="text-2xl font-[900] text-slate-900 tracking-tighter">
                  ₹{doc.fee || 800}
                </span>
              </div>
              <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="bg-slate-900 p-8 text-white">
              <DialogHeader className="mb-8">
                <DialogTitle className="text-3xl font-[900] uppercase tracking-tighter leading-none">
            ...
            <div className="p-8 flex flex-col justify-between">
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 flex items-center">
                    <Clock className="h-4 w-4 mr-2 text-blue-600" /> Choose Time
                    Slot
                  </h4>
