    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <Heart className="h-4 w-4 text-orange-600" /> Manage your companions
            under one account[cite: 46].
          </p>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[500px] border-none rounded-[2.5rem] p-10 bg-white">
            <DialogHeader className="space-y-4">
              <DialogTitle className="text-3xl font-[900] uppercase tracking-tighter">
                New Family Member
              </DialogTitle>
              <DialogDescription className="text-slate-500">
                Provide pet details to start their health passport[cite: 48].
              </DialogDescription>
            </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {petsQuery.data?.map((pet) => (
          <Card
            key={pet.id}
            className="group border-none shadow-xl rounded-[2.5rem] bg-white transition-all hover:scale-[1.02] overflow-hidden"
          >
            <div
              className={`h-32 relative bg-gradient-to-br ${pet.gender === "Male" ? "from-blue-600 to-indigo-700" : "from-pink-500 to-rose-600"}`}
            >
            ...
            <div className="px-8 relative">
              <Avatar className="h-24 w-24 border-4 border-white shadow-xl rounded-[1.8rem] -mt-12">
                <AvatarFallback className="bg-slate-900 text-white font-[900] text-2xl">
                  {pet.name[0]}
                </AvatarFallback>
              </Avatar>
              <div className="mt-6">
                <CardTitle className="text-3xl font-[900] uppercase tracking-tighter text-slate-900">
                  {pet.name}
                </CardTitle>
                <CardDescription className="text-xs font-bold text-blue-600 uppercase mt-1">
...
function MetricItem({ label, val, icon }: any) {
  return (
    <div className="text-center">
      <p className="text-[8px] font-black text-slate-300 uppercase flex items-center justify-center gap-1">
        {icon} {label}
      </p>
