      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <CardHeader className="flex flex-row items-center justify-between p-8 bg-slate-50/50">
            <div>
              ...
              <CardDescription className="font-medium">
                Monitor new participants entering the circle.
              </CardDescription>
            </div>
            <Link href="/admin/users">
              <Button
                variant="ghost"
                className="font-bold text-[10px] uppercase tracking-[0.2em] text-slate-400 hover:text-orange-600"
              >
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {recentRegistrations.map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between p-5 rounded-3xl bg-slate-50 hover:bg-orange-50 transition-all border border-transparent hover:border-orange-100 group"
                  >
        ...
        <div className="lg:col-span-4 space-y-8">
            ...
            <CardContent className="p-8 pt-0 relative z-10">
              <p className="text-slate-400 font-medium text-sm leading-relaxed mb-8">
                ...
                <span className="text-white font-bold">
                  {stats.pendingKyc} pending verification
                </span>{" "}
                requests from Vets and Shops requiring manual document review.
              </p>
          ...
          <div className="space-y-3">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 ml-4 mb-4">
              Quick Management
            </h3>
            <QuickLink
              href="/admin/events"
...
function MetricCard({
  ...
  return (
    <Card
      className={`border-none shadow-lg shadow-slate-200/60 rounded-[2rem] transition-all hover:scale-[1.02] ${highlight ? "ring-2 ring-amber-500/20 bg-amber-50/10" : "bg-white"}`}
    >
      ...
      <CardContent className="px-7 pb-7">
        ...
        <p
          className={`text-[10px] font-black uppercase tracking-widest mt-1 ${highlight ? "text-amber-600" : "text-slate-400"}`}
        >
          {sub}
        </p>
      </CardContent>
...
function QuickLink({ href, icon, label }: any) {
  return (
    <Link
      href={href}
      className="flex items-center justify-between p-5 rounded-[1.5rem] bg-white border border-slate-100 hover:border-orange-200 hover:shadow-xl hover:shadow-orange-100/50 transition-all group"
    >
