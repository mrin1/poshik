...
export default function KYCApprovalsPage() {
  ...
  const getAppsByStatus = (status: string) =>
    filteredApps.filter((app) => app.kyc_status === status);
          ...
          {isLoading ? (
            <TableRow>
              <TableCell colSpan={5} className="py-24 text-center">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-4 italic">
                  Retrieving Node Records...
                </p>
              </TableCell>
            </TableRow>
          ) : data.length === 0 ? (
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-orange-600" /> Oversee all
            registered users and verify platform compliance.
          </p>
        </div>
      ...
      <Tabs defaultValue="ALL" className="w-full">
        <TabsList className="h-16 inline-flex bg-slate-100 rounded-[1.5rem] p-1.5 mb-2">
          <TabsTrigger
            value="ALL"
            className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-slate-900"
          >
            All Registered
          </TabsTrigger>
          <TabsTrigger
          ...
          >
            Pending{" "}
            <Badge className="ml-3 bg-orange-600 text-white border-none text-[9px] h-5 min-w-5">
              {getAppsByStatus("PENDING").length}
            </Badge>
          </TabsTrigger>
          <TabsTrigger
            value="APPROVED"
            className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-emerald-600"
          >
            Approved
          </TabsTrigger>
          <TabsTrigger
            value="REJECTED"
            className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-red-600"
          >
            Rejected
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="PENDING">
          {renderTable(getAppsByStatus("PENDING"))}
        </TabsContent>
        <TabsContent value="APPROVED">
          {renderTable(getAppsByStatus("APPROVED"))}
        </TabsContent>
        <TabsContent value="REJECTED">
          {renderTable(getAppsByStatus("REJECTED"))}
        </TabsContent>
      </Tabs>
        ...
        <DialogContent className="sm:max-w-[600px] border-none rounded-[2.5rem] p-0 overflow-hidden bg-white shadow-2xl">
          <div className="bg-slate-900 p-8 text-white">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black uppercase tracking-tighter flex items-center gap-3">
                <FileText className="h-6 w-6 text-orange-500" /> Identity
                Verification
              </DialogTitle>
              <DialogDescription className="text-slate-400 font-medium italic">
                Reviewing credentials for {selectedApp?.full_name}
              </DialogDescription>
            </DialogHeader>
          </div>
                ...
                {selectedApp?.kyc_document_url ? (
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 hover:border-orange-200 hover:bg-orange-50/30 transition-all group">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      className="font-black text-[10px] uppercase tracking-widest text-orange-600"
                    >
                      <a
                        href={selectedApp.kyc_document_url}
                        target="_blank"
          ...
          <DialogFooter className="p-8 pt-0 flex gap-4">
            {selectedApp?.kyc_status === "PENDING" ? (
              ...
              <Button
                className="w-full h-14 rounded-2xl bg-slate-100 text-slate-900 font-black uppercase text-[10px] tracking-widest hover:bg-slate-200"
