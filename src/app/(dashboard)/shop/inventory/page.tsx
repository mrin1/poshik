...
import { Search, Plus, Edit3, Trash2, Package, AlertCircle, MoreHorizontal, Loader2, Activity, X } from "lucide-react";
...
export default function ShopInventoryPage() {
  ...
  const { inventoryQuery, addMutation, updateMutation, deleteMutation } = useInventory(user?.id);
  ...
  return (
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <Activity className="h-4 w-4 text-emerald-600" /> Managing active SKUs for {user?.full_name}.
          </p>
        </div>
        <Button 
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-100">
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-6 px-8">Product</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-6">Category</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-6">Price</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-6">Stock</TableHead>
            ...
            <TableBody>
              {filteredItems.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-20 text-slate-400 font-bold uppercase text-[10px] tracking-widest">
                ...
                filteredItems.map((product) => (
                  <TableRow key={product.id} className="border-slate-50 hover:bg-slate-50/50">
                    <TableCell className="py-6 px-8">
                      <div className="font-black text-slate-900 uppercase tracking-tight text-sm">{product.name}</div>
                      <div className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest mt-1">SKU: {product.sku || 'N/A'}</div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className="border-slate-100 font-bold text-[9px] uppercase tracking-widest px-3 py-1">
                        {product.category}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-black uppercase tracking-tight ${product.stock <= 5 ? "text-orange-600" : "text-slate-900"}`}>
                          {product.stock} Units
                        </span>
                        {product.stock <= 5 && <AlertCircle className="h-3.5 w-3.5 text-orange-500" />}
                      </div>
                    </TableCell>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <StatBox label="Total SKUs" val={inventory?.length || 0} color="emerald" />
          <StatBox label="Low Stock" val={inventory?.filter(p => p.stock > 0 && p.stock <= 5).length || 0} color="orange" />
          <StatBox label="Out of Stock" val={inventory?.filter(p => p.stock === 0).length || 0} color="red" />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <Card className="w-full max-w-lg border-none shadow-2xl rounded-[2.5rem] bg-white overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              ...
              <Button variant="ghost" size="icon" onClick={() => setIsModalOpen(false)} className="rounded-xl hover:bg-slate-200">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Stock Limit</label>
                  <Input 
                    type="number"
...
function StatBox({ label, val, color }: any) {
  const colors: any = { emerald: "text-emerald-700", orange: "text-orange-700", red: "text-red-700" };
  return (
