...
import { 
  ShoppingBag, Users, TrendingUp, AlertTriangle, Package, 
  ArrowUpRight, ArrowDownRight, Clock, ArrowRight, Sparkles, Activity, Loader2
} from "lucide-react";
...
export default function ShopDashboardPage() {
  ...
  const dynamicStats = [
    { title: "Revenue", value: data.stats.revenue, desc: "Last 30 days", icon: TrendingUp, trend: "up", color: "emerald" },
    { title: "Orders", value: data.stats.orders, desc: "Last 30 days", icon: ShoppingBag, trend: "up", color: "blue" },
    { title: "Customers", value: data.stats.customers, desc: "Unique buyers", icon: Users, trend: "up", color: "purple" },
    { title: "Low Stock", value: data.stats.lowStock, desc: "Attention required", icon: AlertTriangle, trend: Number(data.stats.lowStock) > 0 ? "down" : "neutral", color: "orange" },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in fade-in slide-in-from-top-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-emerald-600" /> Welcome back, {data.shopName}.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <Card className="lg:col-span-8 border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] overflow-hidden bg-white flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between p-8 bg-slate-50/50">
            ...
            <Button onClick={() => handleAction("Navigating to full orders page...")} variant="ghost" className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400 hover:text-emerald-600 active:scale-95 transition-all">
            ...
            <div className="space-y-4">
              {data.recentOrders.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center py-10 opacity-60">
                   ...
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No recent orders found.</p>
                </div>
              ) : (
                data.recentOrders.map((order: any) => (
                  <div key={order.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 rounded-[2rem] bg-slate-50 border border-transparent hover:border-emerald-100 hover:bg-emerald-50/30 transition-all group">
                    ...
                    <div className="flex items-center gap-6 mt-4 sm:mt-0">
                      ...
                      <Button onClick={() => handleAction(`Viewing order ${order.id}`)} variant="ghost" size="icon" className="h-12 w-12 rounded-xl bg-white text-slate-300 hover:text-emerald-600 shadow-sm active:scale-90 transition-all">
                          <ArrowRight size={20} />
                      </Button>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <Card className={`${Number(data.stats.lowStock) > 0 ? 'bg-orange-600' : 'bg-emerald-950'} text-white rounded-[2.5rem] border-none shadow-2xl overflow-hidden p-2 transition-colors`}>
            <CardHeader className="p-8 pb-4">
              <div className={`p-3 w-fit rounded-2xl mb-4 ${Number(data.stats.lowStock) > 0 ? 'bg-orange-500' : 'bg-emerald-500'}`}>
          ...
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white overflow-hidden">
            ...
            <CardContent className="px-8 pb-8 space-y-6">
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <span className="text-xs font-bold text-slate-900 flex items-center gap-2"><Clock size={14} /> Available Now</span>
                </div>
                <div className="text-center py-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Estimated Earnings</p>
                    <h3 className="text-4xl font-[900] text-slate-900 tracking-tighter">{data.payout}</h3>
                </div>
                <Button 
...
const MetricCard = memo(function MetricCard({ title, value, desc, icon: Icon, trend, color }: any) {
  const colors: any = {
