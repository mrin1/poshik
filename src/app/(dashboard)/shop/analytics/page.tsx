...
import { 
  BarChart3, TrendingUp, Users, ShoppingBag, ArrowUpRight, 
  ArrowDownRight, Download, Calendar, Filter, Activity, Sparkles, Loader2
} from "lucide-react";
...
export default function ShopAnalyticsPage() {
  ...
  if (isError || !data) {
    return <div className="p-10 text-center font-bold text-red-500">Failed to load analytics.</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-8 p-4 md:p-8 bg-[#f8fafc] min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <MetricCard title="Revenue" value={data.metrics.revenue} change="+12.5%" isPos icon={TrendingUp} color="emerald" />
        <MetricCard title="Avg Order" value={data.metrics.avgOrder} change="+3.2%" isPos icon={ShoppingBag} color="blue" />
        <MetricCard title="Conversion" value={data.metrics.conversion} change="-0.4%" isPos={false} icon={BarChart3} color="orange" />
        <MetricCard title="Loyalty" value={data.metrics.loyalty} change="+5.1%" isPos icon={Users} color="purple" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <Card className="lg:col-span-8 border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white overflow-hidden">
          <CardHeader className="p-8 pb-4 flex flex-row items-center justify-between">
            ...
            <Button onClick={() => handleAction("Chart filters opened.")} variant="ghost" size="icon" className="rounded-xl bg-slate-50 hover:bg-slate-100 active:scale-90 transition-all">
              <Filter size={16} />
            </Button>
          </CardHeader>
          <CardContent className="p-8">
            <div className="h-[300px] w-full bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-end p-8 overflow-hidden group">
               ...
               <p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 pointer-events-none">Market Dynamics Active</p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-[2.5rem] bg-white overflow-hidden">
              ...
              <CardContent className="px-8 pb-8 space-y-6">
                  <SegmentBar label="Dog Owners" val={data.segments.dogs} color="bg-blue-500" />
                  <SegmentBar label="Cat Owners" val={data.segments.cats} color="bg-purple-500" />
                  <SegmentBar label="Exotic/Others" val={data.segments.others} color="bg-slate-300" />
              </CardContent>
          </Card>
          ...
          <Card className="bg-emerald-950 text-white border-none shadow-2xl rounded-[2.5rem] p-2 overflow-hidden group">
              ...
              <CardContent className="px-8 pb-8 pt-4">
                  ...
                  >
                      {isLaunching ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
...
const MetricCard = memo(function MetricCard({ title, value, change, isPos, icon: Icon, color }: any) {
  const colors: any = {
