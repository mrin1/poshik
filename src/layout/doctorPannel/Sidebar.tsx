    ...
    <aside className="w-80 bg-white border-r border-slate-200 hidden lg:flex flex-col h-[calc(100vh-5rem)] sticky top-20 overflow-hidden shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      ...
      <nav className="flex-1 px-4 py-8 space-y-1 overflow-y-auto no-scrollbar">
        <div className="flex items-center px-4 mb-8">
           ...
           <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Clinical Management</p>
        </div>
        
        {sidebarLinks.map((link) => {
          const Icon = link.icon;
          const isActive = pathname === link.href || (pathname?.startsWith(`${link.href}/`) && link.href !== "/doctor");
          
          return (
            <Link key={link.name} href={link.href} className="group block">
              <div className={`flex items-center justify-between w-full px-4 py-3.5 rounded-2xl transition-all duration-500 relative ${
                isActive 
                ? "bg-slate-900 text-white shadow-2xl shadow-slate-300 translate-x-1" 
                : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"
              }`}>
                <div className="flex items-center">
                  <div className={`p-2 rounded-xl mr-3 transition-colors ${isActive ? "bg-blue-600/20 text-blue-400" : "bg-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600"}`}>
        ...
        <div className="mx-2 mt-12 p-5 rounded-[2rem] bg-gradient-to-br from-blue-600 to-blue-700 text-white relative overflow-hidden shadow-xl shadow-blue-100">
            ...
            <div className="relative z-10">
              <div className="flex items-center mb-3">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest text-blue-100">Live Status</p>
              </div>
              <p className="text-xs font-bold leading-relaxed">
                Clinic active in <span className="underline decoration-blue-300">Kolkata Node</span>. Accepting urgent vet cases.
              </p>
            </div>
        </div>
      </nav>
      ...
      <div className="p-4 bg-slate-50/50 border-t border-slate-100 space-y-2">
        ...
        <div className="flex items-center p-3 mb-2 bg-white rounded-2xl border border-slate-100 shadow-sm">
           ...
           <div className="ml-3 overflow-hidden">
             <p className="text-[10px] font-black text-slate-900 truncate uppercase tracking-tighter">{user?.full_name || "Doctor"}</p>
             <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Verified Vet</p>
           </div>
        </div>
