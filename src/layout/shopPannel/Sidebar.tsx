...
export default function Sidebar() {
  ...
  const sidebarLinks = [
    ...
    { name: "Promotions", href: "/shop/promotions", icon: Tag }, 
  ...
  return (
    <aside className="w-80 bg-white border-r border-slate-100 hidden lg:flex flex-col h-[calc(100vh-5rem)] sticky top-20 overflow-hidden">
      ...
      <nav className="flex-1 px-6 py-8 space-y-2 overflow-y-auto no-scrollbar">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 mb-6 px-4">Merchant Center</p>
        
        {sidebarLinks.map((link) => {
          const Icon = link.icon;
          const isActive = pathname === link.href || (pathname?.startsWith(`${link.href}/`) && link.href !== "/shop");
            ...
            <Link key={link.name} href={link.href} className="group block">
              <div className={`flex items-center justify-between w-full px-5 py-4 rounded-2xl transition-all duration-300 ${
                ...
                <div className="flex items-center">
                  <Icon className={`mr-4 h-5 w-5 transition-transform group-hover:scale-110 ${isActive ? "text-emerald-400" : "text-slate-400"}`} strokeWidth={2.5} />
        ...
        <div className="mt-12 p-6 rounded-[2rem] bg-slate-900 text-white relative overflow-hidden group shadow-lg">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest text-emerald-400 mb-1">Store Status</p>
            <p className="text-[11px] font-[900] leading-tight uppercase tracking-tight">Kolkata Warehouse <br /> is currently live</p>
            <div className="mt-4 flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Accepting Orders</span>
            </div>
        </div>
      </nav>
